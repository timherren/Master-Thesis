...
if provider != "ollama":
    print("   [ERROR] Unsupported LLM_PROVIDER. Use: LLM_PROVIDER=ollama")
else:
    print(
        "   [OK] Local Ollama configured "
...
try:
    ...
    print("   [OK] TRAM-DAG imports successful")
except Exception as e:
    print(f"   [WARN] TRAM-DAG import failed: {e}")
...
try:
    ...
    if application_dir.exists():
        ...
        print("   [OK] R integration available")
    else:
        print(f"   [WARN] R integration directory not found at {application_dir}")
except Exception as e:
    print(f"   [WARN] R integration not available: {e}")
...
for d in dirs:
    ...
    if dir_path.exists():
        ...
        print(f"   [WARN] {d}/ does not exist (will be created on first use)")

# Check FastAPI app
print("\n6. Checking FastAPI app...")
try:
    routes = [r.path for r in app.routes]
    print(f"   [OK] FastAPI app initialized with {len(routes)} routes")
    print(f"   Routes: {', '.join(routes[:5])}...")
except Exception as e:
    print(f"   [ERROR] FastAPI app check failed: {e}")
